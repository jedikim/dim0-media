...
- **Collab engine** (`backend/topix/collab/`; WS handler in `api/router/collab.py`). Invariant: **collab is the only edit path** — every mutation (human WS op or in-process agent) flows through allocate-seq → apply-to-`GraphStore` → broadcast `peer-op`. The backend is a sequencer + relay; the client owns conflict resolution — why + the full contract: **[ADR-SYNC-001](./adr/ADR-SYNC-001-offline-first-relay.md)**.
  ...
  - `agent_bridge.py`: the server agent appears as room client "agent" (`is_system`) without holding a slot. Ordinary agent broadcasts no-op when no room is open (next opener gets the snapshot). Generated-image results are stricter: a live room lock is acquired before any pooled connection, then a generation-scoped PostgreSQL session advisory lock owns Qdrant preparation through final commit without holding a transaction open. Failed `pg_try_advisory_lock` attempts return their connection before a bounded retry and surface as recoverable `materialization_raced` contention. The owned connection commits one deterministic combined node+edge batch and `output_node_uid` binding in a short transaction, then its session lock and connection are released before committed live delivery continues under the room lock. No-room retries recover from `board_oplog`. The batch identity includes both canonical objects' materialization timestamps, so a node-only or edge-only explicit recreation gets one new durable add batch while response-loss retries deduplicate.
...
- **Server image generation** `backend/topix/image_generation/` defines credential-free provider contracts, the static allowlist, confined raster storage, a dedicated OpenRouter `/api/v1/images` adapter, and bounded process-local execution. `POST /boards/{graph_id}/image-generations` authorizes an editor, atomically records a run/attempt/reference snapshots, and returns 202 before provider work; polling and no-store board-scoped asset content use read ACLs. PostgreSQL `(user_uid, board_uid, client_request_uid)` idempotency elects one task winner and rejects a changed fingerprint. Each process owns active work through `worker_uid` + a heartbeated lease; background reconciliation uses a cluster-wide advisory lock and closes only expired work. The no-auto-retry service atomically fails its attempt and run. Generated storage keys are recorded before file writes, and ambiguous DB responses preserve bytes until authoritative reference checks or durable cleanup prove deletion safe. Asset reads support PNG/JPEG/WebP/GIF/AVIF, while provider input/output remains PNG/JPEG/WebP. The semaphore remains process-local until a durable queue exists. Successful runs can materialize one immutable Qdrant result node and ordinary edge; `image_generation_run.output_node_uid` is their nullable durable PostgreSQL association, not graph storage. An already-bound deleted result is never auto-revived, while an editor's explicit bounded recreate restores the same canonical IDs; unbound tombstones remain recoverable partial state.
...
3. **Tauri desktop** (`webui/src-tauri/`) — a standalone app: local-first + offline on the user's own keys, and (optionally) managed AI + synced boards when pointed at a server. Reuses the SPA + the #154 seams. Two load-bearing decisions:
   ...
   - BYOK provider calls go **direct** (CORS-free) via `@tauri-apps/plugin-http`; managed + synced ride the one `API_URL`, driven by a baked `VITE_API_URL` (from the `API_ORIGIN` release variable) or a localStorage override (`features/desktop/`). Contract → **[ADR-DESKTOP-002](./adr/ADR-DESKTOP-002-byok-relay-and-remote-path.md)**.
   - Distribution: `desktop-release.yml` publishes a GitHub Release on a version tag; `install.sh`/`install.ps1` install it prompt-free even unsigned (ad-hoc signed, terminal downloads aren't quarantined). Why → **[ADR-DESKTOP-003](./adr/ADR-DESKTOP-003-distribution-and-unsigned-install.md)**.
