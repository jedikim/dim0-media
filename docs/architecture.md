...
- **Collab engine** (`backend/topix/collab/`; WS handler in `api/router/collab.py`). Invariant: **collab is the only edit path** — every mutation (human WS op or in-process agent) flows through allocate-seq → apply-to-`GraphStore` → broadcast `peer-op`. The backend is a sequencer + relay; the client owns conflict resolution — why + the full contract: **[ADR-SYNC-001](./adr/ADR-SYNC-001-offline-first-relay.md)**.
  ...
  - `agent_bridge.py`: the server agent appears as room client "agent" (`is_system`) without holding a slot. Ordinary agent broadcasts no-op when no room is open (next opener gets the snapshot). Generated-image results are stricter: a generation-scoped PostgreSQL session advisory lock owns Qdrant preparation through final commit without holding a transaction open; the same shared-pool connection then commits one deterministic combined node+edge batch and `output_node_uid` binding in a short transaction. The lock is released before that connection returns to the pool, live peers receive only committed batches, and no-room retries recover from `board_oplog`. The batch identity includes both canonical objects' materialization timestamps, so a node-only or edge-only explicit recreation gets one new durable add batch while response-loss retries deduplicate.
