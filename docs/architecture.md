...
- **Server image generation** `backend/topix/image_generation/` defines credential-free provider contracts, the static allowlist, confined raster storage, a dedicated OpenRouter `/api/v1/images` adapter, and bounded process-local execution. `POST /boards/{graph_id}/image-generations` authorizes an editor, atomically records a run/attempt/reference snapshots, and returns 202 before provider work; polling and board-scoped asset content use read ACLs. PostgreSQL `(user_uid, board_uid, client_request_uid)` idempotency elects one task winner and rejects a changed fingerprint. A provider failure is first preserved on its immutable attempt, then the PR-02 no-auto-retry service terminally fails the run. Startup reconciliation only closes work older than a grace period; concurrency remains per process until a durable queue exists. `image_generation_run.output_node_uid` remains a nullable reservation for the PR-05 canvas result node.
